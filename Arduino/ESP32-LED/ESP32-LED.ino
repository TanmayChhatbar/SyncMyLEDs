/*
  ...
  https://github.com/wirekraken/ESP8266-Websockets-LED/tree/master/ESP8266-LED/data
*/
#include <WebServer.h>
...
void setup() {
  ...
  // settings for your tape (tapes on WS2811, WS2812, WS2812B)
  updateColor(0, 0, 255);
  LEDS.show();
  ...
  if (type == WStype_TEXT) {
    ...
    if (payload[0] == 'B') {
      ...
      if (!flag) {
        ...
        uint8_t r = (0 + (rgb >> 16) & 0xFF);
        uint8_t g = (0 + (rgb >>  8) & 0xFF);
        uint8_t b = (0 + (rgb >>  0) & 0xFF);
